...
Given a foundation in both subjects, I think that reading this book can effectively put some new tools in the reader's toolbelt.
...
As a result, the 3 parts of the book cumulateively build up a technical understanding in order to write clever math-heavy algorithms.
...
## Part 1 - Language Technogogies for Real-Time C++
...
Later on I read a much more thorough book on C++ [[7]](#references), which I'd now reccommend doing *before* reading "Real-Time C++."
...
As far as "optimized" programming goes, Kormanyos's reccomendations are mostly sound.
...
Tools to reduce work at runtime encompass C++ techniques like `constexpr` as well as making good big-O algorithm chioces.
...
Part 2 moves above language fundamentals and moves into writing idomatic C++ building blocks for embedded code.
...
If you're not satisfied, then there's an extraordinary ammount of similar code online that you can reference [[9]](#references).
...
I'd change the `value` from a constant template parameter to a function paramter so that it could be set dynamically too.
...
## Part 3 + Apendicies - Mathematics and Utilities for Real-Time C++
